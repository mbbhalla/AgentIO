...
internal class ContextMemoryManagersTest {
    ...
    @Test
    fun `empty chain should return original conversation`() =
        runBlocking {
    ...
    @Test
    fun `single NoOp CMM should pass through unchanged`() =
        runBlocking {
    ...
    @Test
    fun `CMMs should execute in order, each receiving previous output`() =
        runBlocking {
    ...
    @Test
    fun `CMM that declines a turn should be skipped`() =
        runBlocking {
            val log = mutableListOf<String>()

            val everyOtherTurnCmm =
                object : ContextMemoryManager {
