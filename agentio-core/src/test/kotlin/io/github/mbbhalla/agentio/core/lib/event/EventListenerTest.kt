...
internal class EventListenerTest {
    ...
    private fun buildConfig(eventListeners: EventListeners = EventListeners()): AgentConfiguration {
    ...
    private fun capturingListeners(): EventListeners =
        EventListeners(
            setOf(
                EventListener { event ->
    ...
    @Test
    fun `should emit AgentInvocationStart and AgentInvocationEnd on successful invocation`() =
        runBlocking {
            // Given
    ...
    @Test
    fun `should emit AgentInvocationEnd with error on failure`() =
        runBlocking {
            // Given
            val config = buildConfig(eventListeners = capturingListeners())
            val function = TestAgenticFunction(config)
    ...
    @Test
    fun `should emit BeforeLlmCall and AfterLlmCall around bedrock call`() =
        runBlocking {
            // Given
            val config = buildConfig(eventListeners = capturingListeners())
            val function = TestAgenticFunction(config)
            ...
            coEvery { mockToolsProvider.listTools() } returns emptyList()
            coEvery { mockBedrockClient.converse(any()) } returns
                mockSuccessResponse(
                    inputTokens = 200,
                    outputTokens = 80,
                )
    ...
    @Test
    fun `should emit AfterLlmCall with error when bedrock throws`() =
        runBlocking {
            // Given
            val config = buildConfig(eventListeners = capturingListeners())
            val function = TestAgenticFunction(config)
    ...
    @Test
    fun `should emit BeforeToolCall and AfterToolCall around tool execution`() =
        runBlocking {
            // Given
            val config = buildConfig(eventListeners = capturingListeners())
            val function = TestAgenticFunction(config)
            ...
            val mockToolUseBlock =
                mockk<aws.sdk.kotlin.services.bedrockruntime.model.ToolUseBlock> {
            ...
            val mockToolResultBlock =
                mockk<aws.sdk.kotlin.services.bedrockruntime.model.ToolResultBlock> {
                    every { toolUseId } returns "tool-use-123"
            ...
            coEvery { mockBedrockClient.converse(any()) } returnsMany
                listOf(
                    toolUseResponse,
                    mockSuccessResponse(),
                )
    ...
    @Test
    fun `should emit AfterToolCall with error when tool throws`() =
        runBlocking {
            // Given
            val config = buildConfig(eventListeners = capturingListeners())
            val function = TestAgenticFunction(config)
    ...
    @Test
    fun `should emit events for parallel tool calls`() =
        runBlocking {
            // Given
            val config = buildConfig(eventListeners = capturingListeners())
            val function = TestAgenticFunction(config)
            ...
            val mockToolResultBlock1 =
                mockk<aws.sdk.kotlin.services.bedrockruntime.model.ToolResultBlock> {
                    every { toolUseId } returns "tool-use-a"
                    every { content } returns emptyList()
                    every { status } returns ToolResultStatus.Success
                }
            val mockToolResultBlock2 =
                mockk<aws.sdk.kotlin.services.bedrockruntime.model.ToolResultBlock> {
                    every { toolUseId } returns "tool-use-b"
                    every { content } returns emptyList()
                    every { status } returns ToolResultStatus.Success
    ...
    @Test
    fun `should emit events in correct lifecycle order`() =
        runBlocking {
            // Given
            val config = buildConfig(eventListeners = capturingListeners())
            val function = TestAgenticFunction(config)
    ...
    @Test
    fun `should emit TurnCompleted after each turn`() =
        runBlocking {
            // Given
            val config = buildConfig(eventListeners = capturingListeners())
            val function = TestAgenticFunction(config)
    ...
    @Test
    fun `should emit TurnCompleted with correct turn numbers across multiple turns`() =
        runBlocking {
    ...
    @Test
    fun `should emit TurnCompleted with conversation containing accumulated messages`() =
        runBlocking {
    ...
    @Test
    fun `should work correctly when no event listeners are configured`() =
        runBlocking {
