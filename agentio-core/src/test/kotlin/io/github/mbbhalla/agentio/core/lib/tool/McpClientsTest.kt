...
class McpClientsTest {
    ...
    @Test
    fun `should list tools from all clients`() =
        runBlocking {
            // Given
            val tool1 =
                McpTool(
            ...
            coEvery { mockClient1.listTools() } returns
                ListToolsResult(
                    tools = listOf(tool1),
                    nextCursor = null,
                )
            coEvery { mockClient2.listTools() } returns
                ListToolsResult(
                    tools = listOf(tool2),
                    nextCursor = null,
                )
    ...
    @Test
    fun `should handle tool name sanitization`() =
        runBlocking {
            // Given
            val toolWithSpecialChars =
                McpTool(
    ...
    @Test
    fun `should respect denied tools`() =
        runBlocking {
            // Given
            val allowedTool =
                McpTool(
    ...
    @Test
    fun `should call tool on correct client`() =
        runBlocking {
            // Given
            val tool =
                McpTool(
            ...
            coEvery { mockClient1.callTool(any()) } returns
                CallToolResult(
    ...
    @Test
    fun `should handle tool execution errors`() =
        runBlocking {
            // Given
            val tool =
                McpTool(
        ...
        runBlocking {
            ...
            assertTrue(
                tools
                    .first()
                    .value.name
                    .startsWith("TestSer___"),
            ) // Truncated to 7 chars
        }

    @Test
    fun `should handle long tool names by truncating`() =
        runBlocking {
            // Given
            ...
            assertTrue(
                tools
                    .first()
                    .value.name.length <= McpClients.BEDROCK_MAX_TOOL_NAME_LENGTH,
            )
        }

    @Test
    fun `should handle tool with no required fields`() =
        runBlocking {
            // Given
            val tool =
                McpTool(
    ...
    @Test
    fun `should handle tool call with null input`() =
        runBlocking {
            // Given
            val tool =
                McpTool(
    ...
    @Test
    fun `should handle tool name without joiner`() =
        runBlocking {
            // Given
            val tool =
                McpTool(
    ...
    @Test
    fun `should handle client with null name and null server version`() =
        runBlocking {
            // Given
            coEvery { mockClient1.serverVersion } returns null

            val namedClientWithNulls =
                NamedClient(
                    name = null,
                    client = mockClient1,
                    deniedTools = emptySet(),
    ...
    @Test
    fun `should handle tool result with JSON content`() =
        runBlocking {
            // Given
            val tool =
                McpTool(
    ...
    @Test
    fun `should handle tool result with plain text content`() =
        runBlocking {
            // Given
            val tool =
                McpTool(
