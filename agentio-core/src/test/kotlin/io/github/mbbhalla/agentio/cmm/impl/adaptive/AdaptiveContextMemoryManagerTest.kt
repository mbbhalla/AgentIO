...
internal class AdaptiveContextMemoryManagerTest {
    ...
    @Test
    fun `getContext with single-segment conversation should return unchanged`() =
        runBlocking {
            val cmm = AdaptiveContextMemoryManager()
    ...
    @Test
    fun `getContext with multi-segment conversation should embed probes`() =
        runBlocking {
            val cmm = AdaptiveContextMemoryManager()
    ...
    @Test
    fun `getContext with piggyback enabled should append recall request to last user message`() =
        runBlocking {
            val cmm =
    ...
    @Test
    fun `getContext with piggyback disabled should not append recall request`() =
        runBlocking {
            val cmm =
                AdaptiveContextMemoryManager(
    ...
    @Test
    fun `piggyback recall request should be a separate content block with marker`() =
        runBlocking {
            val cmm =
                AdaptiveContextMemoryManager(
                    config = AdaptiveConfig(enablePiggyback = true),
    ...
    @Test
    fun `measurementFrequency should control when reshuffling occurs`() =
        runBlocking {
            val cmm =
                AdaptiveContextMemoryManager(
                    config =
