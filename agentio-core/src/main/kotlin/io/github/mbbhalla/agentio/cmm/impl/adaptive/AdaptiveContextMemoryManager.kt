...
object DefaultSegmentExtractor : SegmentExtractor {
     ...
     */
    private fun estimateTextImportance(
        content: String,
        isFirst: Boolean,
    ): Double =
        when {
     ...
     */
    private fun estimateToolResultImportance(content: String): Double =
        when {
            content.contains("error", ignoreCase = true) ||
                content.contains("exception", ignoreCase = true) -> 0.85
...
object DefaultSegmentAssembler : SegmentAssembler {
    override fun assemble(
        ...
        val newMessages =
            conversation.messages.map { envelope ->
                if (envelope.message.role != ConversationRole.User) {
                    envelope
                } else {
                    val newContent =
                        envelope.message.content.map { block ->
                            when (block) {
                                is ContentBlock.Text -> {
...
class AdaptiveContextMemoryManager(
    ...
    private var state: AdaptiveState =
        AdaptiveState(
            heatmap = AttentionHeatmap(decayFactor = config.heatmapDecayFactor),
        )

    override fun shouldExecuteOnTurn(turnNumber: Int): Boolean = turnNumber > 0 // Skip turn 0 (initial prompt, no data to measure yet)
    ...
    override suspend fun getContext(input: ContextMemoryManager.ContextMemoryManagerInput): Conversation {
        ...
        val isMeasurementTurn =
            (turnNumber % config.measurementFrequency) == 0 ||
        ...
        val result =
            if (updatedHeatmap.scores.isNotEmpty()) {
