...
interface AgentOutputSelector<O : Any> {
    fun select(agentOutputs: List<Try<AgentOutput<O>>>): Try<AgentOutput<O>>
}
...
class MajorityOccurredAgentOutputSelector<O : Any>(
    ...
    override fun select(agentOutputs: List<Try<AgentOutput<O>>>): Try<AgentOutput<O>> {
...
class AgenticFunctionTrials<I : Instructible.WithInstruction, O : Any>(
    ...
    override suspend fun invoke(input: I): Try<AgentOutput<O>> =
