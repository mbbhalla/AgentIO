...
import com.amazon.agentio.common.listToolsAndSkipDenyList
import com.amazon.agentio.common.toDocument
import com.amazon.agentio.common.toJsonObject
...
data class McpClients(
    ...
    private fun getFullToolName(
         ...
         */
        val newClientIdentifier = clientIdentifier
            .replace(TOOL_NAME_REPLACE_REGEX, "-").take(CLIENT_IDENTIFIER_MAX_LENGTH)

        val newToolName = toolName
    ...
    private suspend fun mapToolNameToClient(): Map<String, Client> {
        return this.set.flatMap { namedClient ->
            namedClient.client.listToolsAndSkipDenyList(
    ...
    private suspend fun toBedrockConverseToolSpec(): List<Tool.ToolSpec> {
        return this.set.flatMap { namedClient ->
            ...
            namedClient.client.listToolsAndSkipDenyList(
                ...
                .map { tool ->
                    val schema = Document.Map(
                        value = mapOf(
                            "type" to Document.String("object"),
                            "required" to Document.List(
                                value = (tool.inputSchema.required ?: emptyList()).map { requiredPropertyName ->
                                    Document.String(requiredPropertyName)
                                },
                            ),
                            "properties" to tool.inputSchema.properties.toDocument(),
                        ),
                    )

                    Tool.ToolSpec(
                        value = ToolSpecification {
                            name = getFullToolName(
                                clientIdentifier = clientIdentifier,
                                toolName = tool.name,
                            )
    ...
    override suspend fun callTool(
        ...
        val toolName = serverAndToolName.split(TOOL_NAME_JOINER).getOrElse(1) {
            serverAndToolName
        }

        val mapToolNameToClient = this.mapToolNameToClient()
        val result = mapToolNameToClient[serverAndToolName]?.callTool(
            CallToolRequest(
                name = toolName,
        ...
        return ContentBlock.ToolResult(
            value = ToolResultBlock {
                ...
                this.content = result?.content?.mapNotNull { (it as TextContent).text }
                    ?.map { text ->
                        val tryJsonObject = Try {
