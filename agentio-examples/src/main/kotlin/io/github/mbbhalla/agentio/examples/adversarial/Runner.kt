...
internal object Runner {
    ...
    fun run(args: Array<String>) = runBlocking {
        val requirements = args.getOrElse(0) {
            """
        ...
        for (iteration in 1..MAX_ITERATIONS) {
            ...
            val designResult = designer.invoke(
            ...
            val designJson = JsonSchemaUtil.json.encodeToString(
                DesignerAgenticFunction.Output.serializer(),
                designOutput,
            )

            LOG.info("[Critic] Reviewing design...")
            val criticResult = critic.invoke(
            ...
            LOG.info("[Critic] Verdict: {} | Issues: {} | Suggestions: {}",
                criticOutput.verdict, criticOutput.criticalIssues.size, criticOutput.suggestions.size)
