...
internal object Runner {
    ...
    fun run(args: Array<String>) = runBlocking {
        ...
        val securityJson = JsonSchemaUtil.json.encodeToString(
            SecurityWorkerAgenticFunction.Output.serializer(),
            securityResult.get().output,
        )
        val qualityJson = JsonSchemaUtil.json.encodeToString(
            QualityWorkerAgenticFunction.Output.serializer(),
            qualityResult.get().output,
        )
        val docsJson = JsonSchemaUtil.json.encodeToString(
        ...
        val finalResult = orchestrator.invoke(
        ...
        LOG.info("\nFull output: {}", JsonSchemaUtil.json.encodeToString(
