...
object FillerCorpus {
    ...
    private val DOMAINS = listOf(
        "marine biology", "urban planning", "medieval history", "quantum mechanics",
        "agricultural science", "volcanic geology", "behavioral economics", "textile manufacturing",
        "orbital mechanics", "culinary chemistry", "forest ecology", "digital typography",
        "bridge engineering", "sleep research", "ceramic arts", "trade logistics",
        "atmospheric science", "dental medicine", "railroad history", "game theory",
        "coral reef systems", "warehouse automation", "folk musicology", "polymer chemistry",
        "arctic exploration", "tax policy", "sports biomechanics", "clock making",
        "river hydrology", "prison reform", "satellite communications", "wine fermentation",
        "earthquake prediction", "childhood education", "submarine design", "postal systems",
        "desert ecology", "pension management", "lighthouse engineering", "seed banking",
    )

    private val SUBJECTS = listOf(
        "researchers", "analysts", "engineers", "practitioners", "scholars", "specialists",
        "investigators", "consultants", "technicians", "administrators", "coordinators",
        "supervisors", "directors", "planners", "architects", "designers", "operators",
        "inspectors", "auditors", "strategists", "theorists", "experimenters", "observers",
        "developers", "managers", "advisors", "examiners", "reviewers", "facilitators",
        "commissioners", "delegates", "representatives", "advocates", "pioneers", "mentors",
    )

    private val VERBS = listOf(
        "examined", "documented", "analyzed", "measured", "compared", "evaluated",
        "investigated", "catalogued", "surveyed", "monitored", "recorded", "assessed",
        "demonstrated", "confirmed", "established", "identified", "observed", "reported",
        "calculated", "estimated", "proposed", "recommended", "implemented", "developed",
        "discovered", "verified", "tested", "validated", "classified", "interpreted",
        "synthesized", "formulated", "constructed", "refined", "optimized", "standardized",
    )

    private val OBJECTS = listOf(
        "the underlying patterns", "the structural properties", "the temporal variations",
        "the spatial distributions", "the causal relationships", "the statistical correlations",
        "the environmental factors", "the operational parameters", "the historical records",
        "the experimental outcomes", "the theoretical predictions", "the observed anomalies",
        "the regulatory frameworks", "the performance metrics", "the resource allocations",
        "the quality indicators", "the risk assessments", "the cost projections",
        "the efficiency ratios", "the compliance standards", "the safety protocols",
        "the maintenance schedules", "the training procedures", "the documentation requirements",
        "the boundary conditions", "the threshold values", "the calibration data",
        "the reference materials", "the baseline measurements", "the control variables",
    )

    private val CONNECTORS = listOf(
        "Furthermore,", "In addition,", "Subsequently,", "Meanwhile,", "Consequently,",
        "Nevertheless,", "Specifically,", "Notably,", "Accordingly,", "Similarly,",
        "In contrast,", "As a result,", "For instance,", "In particular,", "Moreover,",
        "On the other hand,", "At the same time,", "In this context,", "To that end,",
        "With this in mind,", "Building on this,", "Taken together,", "In summary,",
        "Looking ahead,", "By comparison,", "In practice,", "Under these conditions,",
        "Given these findings,", "Along these lines,", "From this perspective,",
    )

    private val ADJECTIVES = listOf(
        "comprehensive", "preliminary", "longitudinal", "systematic", "comparative",
        "quantitative", "qualitative", "empirical", "theoretical", "experimental",
        "observational", "retrospective", "prospective", "cross-sectional", "iterative",
        "incremental", "fundamental", "applied", "interdisciplinary", "collaborative",
        "independent", "standardized", "customized", "automated", "manual",
        "conventional", "innovative", "traditional", "contemporary", "emerging",
    )

    private val QUANTITIES = listOf(
        "approximately 47%", "nearly 83 units", "roughly 12 instances", "about 156 samples",
        "over 2,400 records", "between 30 and 45 cases", "fewer than 200 observations",
        "more than 1,100 data points", "an estimated 67 categories", "precisely 94 measurements",
        "around 350 participants", "up to 78 iterations", "a minimum of 23 trials",
        "no fewer than 560 entries", "exactly 41 configurations", "some 890 specimens",
        "at least 15 variations", "close to 220 assessments", "upwards of 73 factors",
        "a total of 1,850 readings",
    )

    private val TIMEFRAMES = listOf(
        "during the initial phase", "over a three-year period", "in the subsequent analysis",
        "throughout the observation window", "following the preliminary review",
        "prior to the final assessment", "within the first quarter", "across multiple seasons",
        "after extensive calibration", "before the standardization effort",
        "in the early stages", "during peak activity periods", "at regular intervals",
        "over consecutive cycles", "between scheduled evaluations",
    )
    ...
    fun generate(targetTokens: Int): List<String> {
        ...
        val paragraphs = (0 until numParagraphs).map { index ->
    ...
    private fun buildOpeningSentence(rng: Random, domain: String, index: Int): String {
    ...
    private fun buildBodySentence(rng: Random, domain: String): String {
        ...
        return when (rng.nextInt(4)) {
            0 -> "$connector $subject $verb $obj involving $quantity $timeframe, " +
                "yielding $adj results relevant to $domain."
            1 -> "$connector the $adj analysis of $domain revealed that $subject " +
                "$verb $obj encompassing $quantity."
            2 -> "$connector $timeframe, $subject working in $domain $verb $obj " +
                "and documented $quantity through $adj methods."
            else -> "$connector $quantity were attributed to $obj after $subject " +
                "$verb the $adj aspects of $domain $timeframe."
        }
    }

    private fun buildClosingSentence(rng: Random, domain: String): String {
        val adj = ADJECTIVES[rng.nextInt(ADJECTIVES.size)]
        val subject = SUBJECTS[rng.nextInt(SUBJECTS.size)]
        val obj = OBJECTS[rng.nextInt(OBJECTS.size)]
    ...
    private val VOTE_VERBS_FOR = listOf(
        "endorsed", "backed", "supported", "voted in favor of", "expressed approval for",
        "championed", "advocated for", "threw their weight behind", "rallied behind",
        "gave their backing to", "came out in support of", "aligned with",
        "signaled their preference for", "opted for", "chose to support",
        "declared their support for", "affirmed their commitment to",
        "registered their vote for", "indicated a preference for", "stood behind",
    )

    private val VOTE_VERBS_AGAINST = listOf(
        "opposed", "voted against", "rejected", "declined to support",
        "expressed reservations about", "raised objections to", "pushed back against",
        "argued against", "dissented from", "withheld support for",
    )

    private val VOTE_CONTEXTS = listOf(
     ...
     */
    fun generateVoteSentence(index: Int, domain: String, voteForA: Boolean): String {
