...
import io.github.mbbhalla.agentio.experiments.adaptive.function.NeedleRetrievalAgenticFunction
import io.github.mbbhalla.agentio.experiments.adaptive.function.NeedleRetrievalFunctionFactory
...
object Runner {
    ...
    fun run(): List<CellResult> =
        runBlocking {
            ...
            LOG.info("╠══════════════════════════════════════════════════════════════╣")
            LOG.info(
                "║ Cells: {} | Iterations/cell: {} | Total trials: {}",
                cells.size,
                SUITE_CONFIG.numIterations,
                SUITE_CONFIG.totalTrials(),
            )
            ...
            for ((cellIndex, cell) in cells.withIndex()) {
                val cellLabel =
                    "${cell.experimentType.displayName} | ${cell.llm.name} | " +
                ...
                LOG.info(
                    "  Context built: {} blocks, ~{} tokens",
                    needleContext.contextBlocks.size,
                    needleContext.approximateTokens,
                )
             ...
             */
                val evaluationInput =
                    AgenticFunctionEvaluator.EvaluationInput.withFactory(
                        ...
                        onIterationComplete = { progress ->
                            if (progress.result.isFailure) {
                                ...
                                LOG.info(
                                    ...
                                    gt.proposalA,
                                    gt.proposalB,
                                    output.proposalACount,
                                    output.proposalBCount,
                ...
                // Cell summary
                val accuracy =
                    evaluationResult.successAndMatchIterations.toDouble() /
